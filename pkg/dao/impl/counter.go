...
func (impl StatisticsDAOImpl) AddTable(ctx context.Context, info entity.UserInfo) string {
	...
	result, err := impl.db.Exec("insert into user values(?,?,?,?,?,?,?)", info.Id, info.Name, info.Class, info.PhoneNumber, info.ActivityName, info.UserNumber, info.Date)
	if err != nil {
		fmt.Println("新增数据错误")
...
func (impl StatisticsDAOImpl) GetTableByID(ctx context.Context, id string) entity.UserInfo {
	...
	err := result.Scan(&info.Id, &info.Name, &info.Class, &info.PhoneNumber, &info.ActivityName, &info.UserNumber, &info.Date)
	if err != nil {
